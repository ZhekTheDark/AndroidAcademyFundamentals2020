package ru.zhek.androidacademyfundamentals2020.ui.custom
...
class RatingBar @JvmOverloads constructor(
    ...
    private fun getSettingsFromAttr(attrs: AttributeSet) {
        context.obtainStyledAttributes(attrs, R.styleable.RatingBar).run {
            innerPadding =
                getDimension(R.styleable.RatingBar_innerPadding, innerPadding.toFloat()).toInt()
            drawableHeight = getDimension(R.styleable.RatingBar_drawableHeight, 0f).toInt()
            drawableWidth = getDimension(R.styleable.RatingBar_drawableWidth, 0f).toInt()
            isCompensatingMarginActive =
                getBoolean(R.styleable.RatingBar_isCompensatingMarginActive, true)
            recycle()
        }
    }
